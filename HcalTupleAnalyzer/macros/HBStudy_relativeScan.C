...
void HBStudy_relativeScan::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if (HB_energy_byTS[ch_ieta].find(ch_depth) == HB_energy_byTS[ch_ieta].end()) HB_energy_byTS[ch_ieta][ch_depth] = new TH1F(Form("HB_energy_byTS_ieta%d_depth%d",ch_ieta,ch_depth),Form("Energy in HB by TS, 2022 13.6 TeV;QIE11 Digi TS;Energy fraction in each TS (ADC)"),8,0,8);
  ...
  TFile file_out(Form("hcal_histograms_13tev_QIEnominal.root"),"RECREATE");
  ...
  for (std::map<int,TH1F*>::iterator it = HB_SOIratio.begin() ; it != HB_SOIratio.end(); it++) { // it->first is depth, it->second is HB_SOIratio
    ...
    gPad->Update();
    cHB_pulse_shape->SaveAs(Form("2022_plots_relativeScan_Aug/HB_SOIratio_2022_13tev_depth%d.png",it->first));
  }
  ...
  for (std::map<int,TH1F*>::iterator it = HB_SOIminus.begin() ; it != HB_SOIminus.end(); it++) { // it->first is depth, it->second is HB_SOIminus
    ...
    gPad->Update();
    cHB_pulse_shape->SaveAs(Form("2022_plots_relativeScan_Aug/HB_SOIminus_2022_13tev_depth%d.png",it->first));
  }
  // HB_SOI minus energy
  for (std::map<int,TH1F*>::iterator it = HB_SOIminusADC.begin() ; it != HB_SOIminusADC.end(); it++) { // it->first is depth, it->second is HB_SOIminusADC
    ...
    gPad->Update();
    cHB_pulse_shape->SaveAs(Form("2022_plots_relativeScan_Aug/HB_SOIminusADC_2022_13tev_depth%d.png",it->first));
  }
  ...
  for (std::map<int,TH1F*>::iterator it = HB_SOIminus36.begin() ; it != HB_SOIminus36.end(); it++) { // it->first is depth, it->second is HB_SOIminus36
    ...
    //    HB_SOIminus36[it->first]->SetMaximum(0.005); // if 900 gev collisions
    HB_SOIminus36[it->first]->SetMaximum(0.5); // if 13.6 tev collisions
    ...
    gPad->Update();
    cHB_pulse_shape->SaveAs(Form("2022_plots_relativeScan_Aug/HB_SOIminus36_2022_13tev_depth%d.png",it->first));
  }
  ...
  for (int ieta = -16; ieta <= 16; ieta++) {
    ...
    cHB_pulse_4->SaveAs(Form("2022_plots_relativeScan_Aug/HB_pulseshape_2022_13tev_ieta%d.png",ieta)); // somehow this is instead drawing the TDC peak by TS instead, confused. -16 is normal, and then messed up
    ...
    latex->DrawLatex(0.5, 0.95, cmsLabel);
    cHB_firstADC_4->SaveAs(Form("2022_plots_relativeScan_Aug/HB_firstADC_2022_13tev_ieta%d.png",ieta));
    ...
    latex->DrawLatex(0.5, 0.95, cmsLabel);
    cHB_peak_4->SaveAs(Form("2022_plots_relativeScan_Aug/HB_peak_2022_13tev_ieta%d.png",ieta));
    ...
    latex->DrawLatex(0.5, 0.95, cmsLabel);
    cHB_validTDC_4->SaveAs(Form("2022_plots_relativeScan_Aug/HB_validTDC_2022_13tev_ieta%d.png",ieta));
