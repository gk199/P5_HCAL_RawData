...
void HBStudy::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if (HB_energy_ieta[ch_ieta].find(ch_depth) == HB_energy_ieta[ch_ieta].end()) HB_energy_ieta[ch_ieta][ch_depth] = new TH1F(Form("HB_energy_ieta%d_depth%d",ch_ieta,ch_depth),Form("SOI + SOI+1 energy in HB, 2022 900 GeV (with ADC>64) (ieta=%d,depth=%d);QIE phase setting;Energy Ratio",ch_ieta,ch_depth),120,1,121);
	...
	if ((QIE11DigiADC->at(ch).at(2) <= 10) && ((QIE11DigiADC->at(ch).at(3) > ADCenergy) || (QIE11DigiADC->at(ch).at(4) > ADCenergy))) {
	  ...
	  HB_energy_ieta[ch_ieta][ch_depth]->Fill(laserType, (SOI_energy + SOIp1_energy)); // SOIm1_energy + SOI_energy + SOIp1_energy
