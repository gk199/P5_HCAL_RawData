#define HBStudy_LocalQIEscan_nominal_cxx
#include "HBStudy_LocalQIEscan_nominal.h"
#include <TH2.h>
#include <TF1.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void HBStudy_LocalQIEscan_nominal::Loop()
{
  //   In a ROOT session, you can do:
  //      root> .L HBStudy_LocalQIEscan_nominal.C
  //      root> HBStudy_LocalQIEscan_nominal t
  ...
  TFile file_out(Form("hcal_histograms_13tev_QIE_LocalQIEscan_nominal.root"),"RECREATE");
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if (laserType != 0) continue; // only do this for "new nominal" QIE phases, which have laser type of 0
  ...
  for (int TS = 2; TS <= 4; TS++) {
	...
	gPad->Update();
      ...
      //      cHB_promptTDC_4->SaveAs(Form("FlatLUTs/HB_%dTDC_by_ieta_2022_13tev_TS%d_run%d.png",TDC,TS,runNum));
    } // TDC loop
    ...
    latex->DrawLatex(0.35, 0.95, cmsLabel);
    cHB_stackedTDC->SaveAs(Form("FlatLUTs/TDC_by_ieta_2022_13tev_TS%d_run%d.png",TS,runNum));
    ...
    latex->DrawLatex(0.35, 0.95, cmsLabel);
    //    cHB_stackedTDC->SaveAs(Form("FlatLUTs/TDC_by_ieta_2022_13tev_TS%d_run%d_log.png",TS,runNum));
  } // end TS loop
}
